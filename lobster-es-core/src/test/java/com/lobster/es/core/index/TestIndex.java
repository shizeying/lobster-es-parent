...
public class TestIndex {
    ...
    @IndexField(fieldName = "dense_vector_field", type = FieldType.DENSE_VECTOR, dims = 128,
        index = DocValuesOption.TRUE, similarity = Similarity.L2_NORM)
    ...
    @IndexField(fieldName = "join_field", type = FieldType.JOIN, joinType = JoinTypeObj.class,
        relations = {@JoinTypeRelations(parent = "parent_field", child = {"child_field", "child_field3"}),
    ...
    @IndexField(fieldName = "rank_feature_field", type = FieldType.RANK_FEATURE,
        positive_score_impact = DocValuesOption.TRUE)
    ...
    @IndexField(fieldName = "text_field", type = FieldType.TEXT, analyzer = "standard", search_analyzer = "standard",
        fielddata = DocValuesOption.TRUE,
        fielddata_frequency_filter = @FielddataFrequencyFilter(min = 2, max = 100, minSegmentSize = 1000))
    @Fields(
        fields = {@IndexField(fieldName = "keywordfield", type = FieldType.KEYWORD, fielddata = DocValuesOption.TRUE)})
    ...
    @IndexField(fieldName = "aggregate_metric_double_field", type = FieldType.AGGREGATE_METRIC_DOUBLE,
        metrics = {Metrics.MIN, Metrics.MAX, Metrics.SUM, Metrics.VALUE_COUNT},
