...
public class TestIndex {
    ...
    @IndexField(fieldName = "join_field", type = FieldType.JOIN, joinType = JoinTypeObj.class)
    ...
    @IndexField(fieldName = "rank_feature_field", type = FieldType.RANK_FEATURE, positive_score_impact = true)
    ...
    @IndexField(fieldName = "token_count_field", type = FieldType.TOKEN_COUNT)
    private int tokenCountField;

    @IndexField(fieldName = "version_field", type = FieldType.VERSION)
    private long versionField;

    @IndexField(fieldName = "text_field", type = FieldType.TEXT, analyzer = "standard", search_analyzer = "standard", fielddata = true, fielddata_frequency_filter = @FielddataFrequencyFilter(min = 2, max = 100, minSegmentSize = 1000))
    private String textField;

    @IndexField(fieldName = "keyword_field", type = FieldType.KEYWORD, ignore_above = 256, norms = true)
    ...
    @IndexField(fieldName = "aggregate_metric_double_field", type = FieldType.AGGREGATE_METRIC_DOUBLE, metrics = {Metrics.MIN, Metrics.MAX, Metrics.SUM, Metrics.VALUE_COUNT}, default_metric = Metrics.VALUE_COUNT, time_series_metric = TimeSeriesMetric.GAUGE)
    private double aggregateMetricDoubleField;

    @IndexField(fieldName = "alias_field", type = FieldType.ALIAS, path = "original_field")
